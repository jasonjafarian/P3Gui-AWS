define([
    ...
    module.service('filterSrv', function(dashboard, ejsResource, sjsResource,
                                         querySrv, $location, $timeout, $rootScope) {
        ...
        this.update_url = function() {
            if (self.loaded === true) {
                var query_string = btoa(this.getSolrFq()+"&"+querySrv.getORquery());
                $location.search('query', query_string);
            }
        };
        ...
        this.getSolrFq = function(noTime) {
            ...
            _.each(self.list, function(v,k) {
                ...
                if (v.type === 'time') {
                    ...
                    if (v.mandate === 'must') {
                        filter_fq = filter_fq + '&fq=' + v.field + ':%22' + v.value + '%22';
                    } else if (v.mandate === 'mustNot') {
                        filter_fq = filter_fq + '&fq=-' + v.field + ':%22' + v.value + '%22';
                    } else if (v.mandate === 'either') {
                        filter_either.push(v.field + ':%22' + v.value + '%22');
                    }
                } else if (v.type === 'field') {
                    console.debug('v is set to field:', v);
                    ...
                    if (v.mandate === 'must') {
                        ...
                        console.debug("value " + v + "set with mandate of either");
            ...
            if (filter_either.length > 0) {
                console.debug('filter_either is set to:', _.uniq(filter_either));
                filter_fq = filter_fq + '&fq=(' + _.uniq(filter_either).join(' OR ') + ')';
            }
